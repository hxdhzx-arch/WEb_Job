...
var _lastResumeText="";
function submitResume(){var consent=document.getElementById("privacy-consent");if(consent&&!consent.checked){showError("请先勾选同意隐私政策和用户协议");return;}
  // Credit check
  if(typeof PowerSystem!=="undefined"){PowerSystem.requirePower().then(function(ok){if(ok)_doSubmitResume();});return;}
  _doSubmitResume();
}
function _doSubmitResume(){var btn=document.getElementById("submit-btn"),rp=document.getElementById("result-panel"),text=getResumeText();if(!text){showError("请输入");return;}_lastResumeText=text;btn.disabled=true;btn.innerHTML='<span class="loading-spinner"></span>分析中...';rp.classList.remove("visible");fetch("/api/analyze-resume",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({resume_text:text})}).then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error);return d;});}).then(function(d){renderScoreRing("score-container",d.overall_score||0);renderDimensions("dimensions-container",d.dimensions);renderList("highlights-container","亮点",d.highlights,"green","green");renderList("issues-container","待改进",d.issues,"red","red");renderList("suggestions-container","建议",d.suggestions,"blue","blue");renderSummary("summary-container","AI改写简介",d.rewritten_summary);var ps=document.getElementById("polish-section");if(ps)ps.style.display="block";rp.classList.add("visible");rp.scrollIntoView({behavior:"smooth"});PowerSystem.promptReview("resume_diag");}).catch(function(e){showError(e.message);}).finally(function(){btn.disabled=false;btn.innerHTML="开始诊断";});}
var _missingKeywords=[];
function submitJdMatch(){var consent=document.getElementById("privacy-consent-jd");if(consent&&!consent.checked){showError("请先勾选同意隐私政策和用户协议");return;}
  if(typeof PowerSystem!=="undefined"){PowerSystem.requirePower().then(function(ok){if(ok)_doSubmitJdMatch();});return;}
  _doSubmitJdMatch();
}
function _doSubmitJdMatch(){var btn=document.getElementById("submit-btn"),rp=document.getElementById("result-panel"),rt=getResumeText(),jt=document.getElementById("jd-input").value.trim();if(!rt||!jt){showError("请填写");return;}_lastResumeText=rt;_missingKeywords=[];btn.disabled=true;btn.innerHTML='<span class="loading-spinner"></span>匹配中...';rp.classList.remove("visible");fetch("/api/match-jd",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({resume_text:rt,jd_text:jt})}).then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error);return d;});}).then(function(d){
